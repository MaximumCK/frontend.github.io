...
export class InformationService {
    ...
    getProducts(data) {
        return this.http.get<any>(`/informationgetView?type=${data}&search=''`)
        .toPromise()
        .then(res => <Product[]>res.data)
        .then(data => { 
            for(let i=0; i<data.length; i++){
                if(data[i].INFORMATION_TYPE == '1'){
                    ...
                    data[i].INFORMATION_TYPE='ที่กิน'
                }else if(data[i].INFORMATION_TYPE == '4'){
                    data[i].INFORMATION_TYPE='ที่พัก'
                }
            }
            return data; });
    }
